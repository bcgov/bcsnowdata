...
snow_daily_archive_ <- function(){
  ...
  archive = httr::content(res, encoding = "UTF-8", type = "text/csv",
  ...
  #archive$Date = as.Date(archive$Date, format = "%Y-%m-%d")
...
snow_auto_archive_ <- function(parameter_id = c("SWE", "Snow_Depth", "Precipitation", "Temperature")){
  ...
  tbl = httr::content(res_archive, encoding = "UTF-8", type = "text/csv",
                              col_types = readr::cols(.default = readr::col_double(),
                                                      `DATE(UTC)` = readr::col_datetime()), progress = FALSE)
...
snow_auto_current_ <- function(parameter_id = c("SWE", "Snow_Depth", "Precipitation", "Temperature")){
  ...
  tbl = httr::content(res_current, encoding = "UTF-8", type = "text/csv",
                              col_types = readr::cols(.default = readr::col_double(),
...
snow_manual_archive_ <- function(){
  ...
  data_manual <- httr::content(res, encoding = "UTF-8",
                col_types = readr::cols(
